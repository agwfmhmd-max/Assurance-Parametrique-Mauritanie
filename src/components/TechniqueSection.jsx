...
function BasisRiskPanel({ lang }) {
  ...
  const simpleExplain = lang === "ar"
    ? "بلغة بسيطة: قد يشير المؤشر المناخي إلى «لا مشكلة» بينما يتكبد المزارع خسارة فعلية على أرضه (أو العكس). هذه الفجوة بين ما يقيسه المؤشر وما يحدث ميدانيًا تسمى «خطر الأساس»، وهي أكبر نقطة ضعف بنيوية في التأمين التأشيري."
    : "En langage simple : l'indice climatique peut indiquer « tout va bien » alors que l'agriculteur subit une vraie perte sur sa parcelle (ou l'inverse). Cet écart entre ce que mesure l'indice et ce qui se passe réellement sur le terrain s'appelle le « risque de base » — c'est la principale faiblesse structurelle de l'assurance indicielle.";
  return <Card><div className="flex items-center gap-2 mb-3"><AlertTriangle size={18} style={{ color: C.orange }} /><h3 className="text-lg font-bold" style={{ color: C.navy }}>{lang === "ar" ? "تحليل خطر الأساس" : "Analyse du Basis Risk"}</h3></div><p className="text-sm leading-relaxed mb-3 rounded-lg p-3" style={{ color: C.navy, background: C.orangeSoft }}>{simpleExplain}</p><p className="text-sm leading-relaxed mb-6" style={{ color: C.slate }}>{lang === "ar" ? "يقيس خطر الأساس الفجوة بين المؤشر المناخي والخسارة الفعلية. لا تعرض المنصة نسبة غير محسوبة." : "Le Basis Risk mesure l'écart entre l'indice climatique et la perte réelle. Aucun pourcentage n'est affiché sans calcul documenté."}</p><div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-3">{labels.map((label) => <Metric key={label} icon={ShieldAlert} label={label} value={unavailable(lang)} tone={C.orange} lang={lang} />)}</div></Card>;
}
